...
load_deaths <- function(){
  ...
  download_all_data() %>% 
    ...
    dplyr::rename(confirmed_deaths = total_deaths, Date = date, `Country/Region` = location) %>% 
...
load_confirmed <- function(){
  ...
  download_all_data() %>% 
    ...
    dplyr::mutate(location = ifelse(location == "United States", "US", location)) %>% 
    dplyr::rename(confirmed_cases = total_cases, Date = date, `Country/Region` = location) %>% 
    data.table::data.table()
}
